...
fn run_priority_workload(
    ...
    let completion = recovery_completion::Target {
        control_id: recovery_control,
        root_id: &p2_root_id,
        source_path: &p2_source_root,
        entry_count: P2_SOURCE_ENTRIES,
        live_root_id: &p1_root_id,
        live_count: SAMPLE_COUNT,
        deadline: workload_started + Duration::from_secs(300),
    };
    if completion_boundary == CompletionBoundary::PublishedBaseline {
        recovery_completion::drive_to_publication(
            &mut fixture.production,
            &storage,
            &progress_connection,
            &completion,
            &mut poll,
    ...
    if completion_boundary == CompletionBoundary::PublishedBaseline {
        recovery_completion::verify_reopened(&storage, &progress_connection, &completion);
    }

...
| UX-04B | Rewrite fixture with identical size/ID/mtime while the old request is pending; reconcile one path and show new pixels, reject old publication | [Post-decode same-metadata replacement passes](#controlled-query-source-and-viewer-evidence); [existing path-work retirement and current-pixel convergence](r2c-retained-runtime.md#changed-source-preview-admission-during-existing-path-work) add generated pending/executing-owner evidence; [native watcher publication, stale Dart result retirement and current decoded color pass](r2c-browsing-diagnosis.md#native-replacement-and-exclusive-source-recovery); Release remains open |

...
| UX-05C | Same-path source rewrite, then authoritative rename/removal; newest pixels and stable asset until authoritative removal | [Selected native Debug source identity and natural viewer return pass](r2c-query-interactions.md#settled-source-native-result) after actual source reads retire, with exact catalog/source and normal-exit checks. Earlier failed methods and C12 engine output remain explicit; warning-free final acceptance and Release remain open |

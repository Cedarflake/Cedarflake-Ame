...
| UX-07C | Interrupt/reopen exact leases and exhaust recovery retry; retain durable failure/lineage and keep other roots eligible | [Controlled runtime restart, exhausted-candidate barrier and peer eligibility pass](#startup-query-folder-and-recovery-evidence); [actual EXE loss after one raw inventory batch](r2c-process-lifecycle.md#native-restart-after-an-incomplete-raw-inventory-batch) preserves lineage, complete membership and normal resumed exit. The original exact-leased capture fails because work yielded before termination; real executing-lease expiry and combined exhaustion remain open |

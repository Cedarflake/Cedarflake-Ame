...
| UX-03B | Expand a folder, change revision, load another folder page; replace obsolete window and append only at the same revision | [Eight folder-controller cases pass](#startup-query-folder-and-recovery-evidence); [native 200-child replacement and same-revision append pass](r2c-query-interactions.md#native-folder-window-across-publication); observed sidebar jump and Release input remain open |
...
| UX-04C | Hold source exclusively or present corrupt bytes, then recover; preserve precise failure, valid source retries and no stale ready publication | [Controlled exclusive-open, corruption and newer-source recovery pass](#controlled-query-source-and-viewer-evidence); [native exclusive-open failure, pointer Retry and unchanged-source pixels pass](r2c-browsing-diagnosis.md#native-replacement-and-exclusive-source-recovery); narrow feedback C10 and Release remain open |
...
| UX-08A | Jump by scrollbar/time rail then reverse before completion; immediate visible demand and no old seek rollback | Controlled time-navigation cases; [Release rail clicks load without a wheel trigger and completed warm reversal is observed](r2c-release-native.md), pending-request reversal remains open |

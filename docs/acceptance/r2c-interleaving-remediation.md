...
| R2C-C13 | UX-07C recovery feedback / S1, active | [Native exhaustion and restart](r2c-process-lifecycle.md#c13-exhausted-recovery-notification) preserve the exhausted control and usable cached/peer images, but notification text still promises automatic checking and calls terminal retry-wait work scheduled retry. The status-to-notification policy lacks blocked precedence; do not alter retry or scan authority to make the promise true. | Truthful recovery feedback, complete UX-07C and candidate acceptance |

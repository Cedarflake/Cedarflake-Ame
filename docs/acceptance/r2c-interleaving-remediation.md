...
| R2C-C06 | UX-02B/08C task controls / S1 finding, focused correction verified | The [input-control record](r2c-input-controls.md) preserves the original missing Cancel regression during `pausing`, the presentation admission correction and late-Started Cancel precedence checks. Graphical input reaches a real three-image import, then stops at an independently reproduced diagnostic read-attribution error. Actual keyboard Retry and Pause/Cancel remain unproved. | Actual task controls and final-source gates remain open |
| R2C-C07 | UX-02/08C committed display Retry / S1 finding, focused correction verified | Primary first-publication and Retry bypassed committed admission, allowing passive refresh to supersede them or overlap an earlier read. The [owning correction](r2c-input-controls.md#primary-committed-refresh-admission) reuses the existing coordinator and passes all 23 focused cases, preserving actual failures and waiting for the latest query after supersession. This is separate from diagnostic read attribution; no new source scan or scheduler policy is added. | Native task controls and final-source candidate gates remain open |

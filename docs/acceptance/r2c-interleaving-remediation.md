...
| R2C-C05 | UX-03A/04A/05A/08A browsing / S1, causality unresolved | Direct timeline navigation leaves a blank wall until another scroll; an idle 2012-image middle viewport is independently observed blank. Reported removal/navigation failures remain distinct from the two reproduced rail/viewer presentation defects. The [native observation record](r2c-live-gap-recovery.md#c05-browsing-findings--unresolved) preserves the pre-removal database; the [bounded diagnosis](r2c-browsing-diagnosis.md) records passing controlled navigation, corrected test assumptions and the unknown-cause stop. No shared causality or C05 product repair is claimed. | Item 3, usable browsing and candidate readiness; native-evidence pivot requires a scope decision |

...
| R2C-C09 | UX-03B folder browsing / S2, registered | The first native Show More after a revision change replaces correct membership but resets sidebar scroll to the top. The [folder record](r2c-query-interactions.md#native-folder-window-across-publication) preserves the actual input and rendered positions. Diagnose branch-loading retention before correction; no source or catalog membership loss is observed. | Sidebar position continuity; retained for the later S2 pass |

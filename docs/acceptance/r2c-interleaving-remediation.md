...
| R2C-C12 | UX-05C graphics diagnostic / S2, deferred to final minor-repair batch | [The selected source-identity lifetime](r2c-query-interactions.md#c12-graphics-diagnostic-remains-open) meets its functional, source and process assertions but emits `Trying to use texture on two GrContexts!`. No corresponding UI failure is observed; product-versus-observer ownership remains unproved. Retain the warning without suppression or a clean-engine claim. | Warning-free candidate/client acceptance |

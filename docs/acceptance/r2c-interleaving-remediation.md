...
| R2C-C05 | UX-03A/04A/05A/08A browsing / S1, native causality unresolved | The [bounded diagnosis](r2c-browsing-diagnosis.md) proves and corrects application preview-demand loss after cancelled/rejected work, with 100 focused tests and lint passing. Its one native session is consumed by input-tool refusals; the user confirms no window operation. The [native blank observation](r2c-live-gap-recovery.md#c05-browsing-findings--unresolved), reported gray/error sequence and two reproduced presentation defects remain open. This correction does not establish shared causality or a repaired native replay. | Item 3, usable browsing and candidate readiness; renewed diagnosis stopped with native replay outstanding |

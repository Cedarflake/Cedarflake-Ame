...
| R2C-C14 | UX-02B first-import Pause / S1 finding, focused correction verified | [Opening retirement](r2c-retained-runtime.md#first-import-opening-retirement-correction) reproduces the hosted failure after Pause revokes selected capture work. The opening owner returns typed retirement without removing transaction, generation or execution-permit checks; committed and unrelated error outcomes remain distinct. Focused tests and independent review pass. | Connected native Pause, complete Daily, unsigned/hosted and candidate gates |

...
| R2C-C05 | UX-03A/04A/05A/08A browsing / S1 finding, selected generated-client paths verified | The [diagnosis and current evidence](r2c-browsing-diagnosis.md) retains original failures and owning corrections for preview demand, capture dates, rail/viewport publication, viewer geometry and missing-source request retirement. The complete generated bulk lifetime passes current pixels, rail/viewer checks and 4506 observed frames without Retry. It does not establish all frozen client paths, Release input or final local gates. | Remaining variants and candidate gates; selected generated browsing path verified |
| R2C-C06 | UX-02B/08C task controls / S1 finding, focused/native correction verified | The [input-control record](r2c-input-controls.md#complete-native-input-and-rendered-feedback) preserves the original missing Cancel regression, diagnostic failures and their corrections. Actual pointer Pause/Cancel, late-Started Cancel precedence and rendered feedback now pass with exact retained membership and owned exit. Keyboard Cancel and complete focus-return variants remain separate. | Remaining keyboard/Release variants and final-source gates |
| R2C-C07 | UX-02/08C committed display Retry / S1 finding, focused/native correction verified | Primary first-publication and Retry bypassed committed admission, allowing passive refresh to supersede them or overlap an earlier read. The [owning correction](r2c-input-controls.md#primary-committed-refresh-admission) reuses the existing coordinator and passes 23 focused cases. [Focused native Enter](r2c-input-controls.md#complete-native-input-and-rendered-feedback) admits exactly one committed display retry without a source rescan; query supersession remains controlled-test evidence. | Remaining Release paths and final-source candidate gates |

### Current native interaction findings

| ID | Scenario / severity | Evidence and owning obligation | Blocked exit |
| --- | --- | --- | --- |
| R2C-C08 | UX-03A continuous search / S1 finding, focused/native correction verified | [Native input and connected regressions](r2c-query-interactions.md#c08-uninterrupted-search-input) prove query loading disabled SearchBar and lost focus. The availability policy preserves command exclusions while ordinary reads remain editable. Actual `2`/`0`/`0` keys retain focus without another pointer action and settle to the exact current image; source and normal-exit checks pass. Supersession/failure remain connected-test evidence. | Affected final-source and Release gates |
| R2C-C09 | UX-03B folder browsing / S2, focused and native functional correction verified | The original native Show More jump is reproduced as temporary tree clearing. [Window retention and retirement](r2c-query-interactions.md#c09-retained-folder-windows-during-revision-replacement) preserve position and cursor/root/subtree/late-result authority. Native replacement retains the actual offset through 50 pending frames and publication, followed by exact append. Original failures remain. | Complete native lifetime timing and candidate gates |
...
| R2C-C11 | UX-08A loading feedback / S2, focused and selected native correction verified | [The retained causal failures](r2c-query-interactions.md#c11-loading-row-geometry-during-pending-reversal) identify a two-pixel loading-row viewport change and one-pixel center-anchor correction. [Stable loading composition](r2c-query-interactions.md#c11-stable-loading-region-verification) preserves child constraints and input, passes 61 focused cases and the unchanged native pending-reversal assertion: exact new target, 16 current visible images and ten seconds without position correction. [Current Release rail and settled point observations](r2c-release-native.md#current-release-gallery-stability-and-incomplete-menu-traversal) add optimized-client evidence without claiming continuous frame stability. Normal exit and full source/catalog checks pass. | Final-source, remaining Release variants and candidate acceptance |
| R2C-C12 | UX-05C graphics diagnostic / S2, selected observer correction verified | [Contextual attribution and corrected complete workflow](r2c-query-interactions.md#c12-raster-observer-and-complete-source-identity-result) localize the warning to the ignored observer's direct pixel readback. Raster-owned sampling preserves current-image assertions and completes rewrite/rename/removal with empty stderr, source integrity and normal exit. Earlier failures remain; product code is unchanged and no global clean-engine claim follows. | Final-source, Release and candidate acceptance |
| R2C-C13 | UX-07C recovery feedback / S1 finding, focused/native correction verified | [Native exhaustion and restart](r2c-process-lifecycle.md#c13-exhausted-recovery-notification) reproduce an automatic-recovery promise for exhausted work. The extracted feedback policy prioritizes blocked state and describes unfinished work without inventing retry/scan authority. Eight focused cases, 20 existing cases, full lint, independent review and one retained-state native lifetime pass; correct feedback, cached/peer pixels, unchanged failed lineage, source safety and normal exit are preserved. | Final-source, Release, complete UX-07C and candidate acceptance |
| R2C-C14 | UX-02B first-import Pause / S1 finding, owning correction and connected native path verified | [Opening retirement](r2c-retained-runtime.md#first-import-opening-retirement-correction) reproduces the hosted failure after Pause revokes selected capture work. Typed retirement preserves transaction, generation, execution-permit and unrelated error rules. Focused tests, independent review, lint, complete local Daily, connected native Pause/state restoration/Cancel and current-source hosted Windows Scan/unsigned gates pass. | Remaining hosted aggregate, final candidate and separate Release/client obligations |
...
`935961800893862a2a2d0cf50ce6f31673ce3f12`, finishes with nine required jobs passing and Static/Rust
plus the aggregate failing. Three signing-only jobs are skipped. Rust reports 1517 passed,
one failed and 19 ignored in 1217.40 seconds. The failing
`same_mixed_load_controls_poll_connection_lifetime_without_relaxing_the_product_gate` reaches the

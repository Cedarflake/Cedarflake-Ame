...
| R2C-C04 | UX-03A/06A live bulk removal / S1 | With a frozen 10000-image background, 2000 generated additions publish correctly. Deleting 1500 of the 2012 stimulus files leaves all 2012 catalog items after 300 seconds, though only 512 files remain. Twelve coalesced LiveOnly subtree tasks reach eight attempts with no next retry and `metadata_inventory_required`; no new P2 inventory is created. The queue promotion path requires a journal opening boundary for these subtrees, while its root-gap path permits journal-free recovery. Existing exhausted rows also lack an eligible consumer. This is distinct from C01's performance failure. The cycle record preserves native, queue and source evidence; the fourth-family repair proposal awaits its scope decision. | Item 3, exact live removal, candidate readiness |

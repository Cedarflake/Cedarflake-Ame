...
#### Source verification cost and native admission

Reserve 60 active minutes, cumulative 8674 reserved: ten for a read-only cost comparison, 20 for
the narrow verification owner and negative checks, five independent admission review, 15 for at
most one native lifetime, and ten postchecks/review/records. The prior 8579 minutes remain charged;
the separate 35-minute disposable-source deletion admission remains conditional. The previous
copy log reports 59 seconds while copy plus verification takes 127772 ms. Compare hashing on the
same generated bytes before changing source preparation: the installed default SHA-256 type is
managed, while Windows CNG is available. Separate hash computation, filesystem reads and native
operation/observation gaps; do not assume a faster hash fixes every admission cost.

Keep copying and all source/path, unique membership, length, creation/modified date, abort and
deadline checks. If measurements support it, extract only the bounded read-only digest operation;
prove digest equivalence and failure/stream retirement with dedicated fixtures. No source writes,
additional corpus copy, broad preparation rewrite or parallel hashing is admitted. Check the full
10000-file corpus against its frozen oracle before using the new method. A host diagnostic is
causal preparation evidence, not guest performance or functional acceptance.

Only after measured improvement and independent helper review may one current Release lifetime
run with the preceding observed photo-focus route. Keep all 10000 files/10921494393 bytes, exact
historical dates, ordinary guest NTFS/Known Folders, seven-GiB entry, 3072-MiB guest, two-GiB app/host
bounds, 240-second import admission, 300-second import completion, 540-second optional cutoff,
900-second parent and six-second normal app close. Observe actual import completion, decoded
photos and more/layout Enter/Escape/Enter without pointer refocus. Retain finalization observations
without claiming a continuous frame trace. Require exact closed membership, complete guest/host
source checks and independent process retirement. Preserve any failure; no unchanged replay or
deadline extension is admitted. This does not close the older peer-publication incident or admit
the pending source deletion.

The [native result](../acceptance/r2c-release-native.md#bounded-digest-native-import-and-menu-return)
passes this selected boundary: preparation takes 101972 ms, picker confirmation occurs at 195.992
host seconds, actual completed import and decoded photos are recorded within 59.457 seconds, and
more/layout both complete native Enter/Escape/Enter without pointer refocus. Exact closed membership,
complete guest/10516-file host source checks and normal application/Job/Sandbox retirement pass.
The post-close window capture error remains explicit alongside successful input return
and independent retirement. This does not measure the earlier concurrent finalization interval.
Charge all 60 minutes: 8639 charged, 8674 reserved, with only the separate conditional 35-minute

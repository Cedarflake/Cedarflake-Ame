...
Latest planning direction: 2026-09-09 — bounded critical-experience investigation before further fixes
...
The 2026-09-09 browsing regression has a committed implementation checkpoint for preview
source-version recovery, coherent gallery refresh, and folder pagination during catalog publication.
Its [verification record](acceptance/synchronization-browsing-recovery.md) retains the initial Daily
failure, subsequent focused and partition results, and unsigned Windows evidence. The unchanged
mixed-load isolated pass does not establish full-suite stability or close item 2. A fresh complete
Daily invocation and current retained-library client acceptance remain unproved.

The 2026-09-09 planning direction replaces immediate symptom-by-symptom implementation with the
bounded investigation below. Its discovery pass covers the workflows in items 2–3 before further
product fixes; implementation still follows this queue with item 2 active. This is an explicit
ordering clarification, not acceptance of item 2 or permission to start another delivery stage.
...
2. **Active — mixed-load convergence and latency; investigation before further fixes.** Resolve the
   recorded slow catalog-open, observation, and retirement paths using the existing operation
   measurements. Keep the original
...
#### Bounded critical-experience investigation — 2026-09-09

Status: **planned; execution has not started**. This section is the execution plan for the existing
closeout queue, not a second product stage. Its outcome is one reviewed candidate with evidence for
the fixed workflows below, plus an explicit disposition of every discovered issue. It does not
